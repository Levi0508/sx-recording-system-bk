...
export const PageRegister = () => {
  ...
  const onFinish = async (values: {
    ...
    const invitationCode = localStorage.getItem('__INVITATION_CODE')

    const resp = await services.user$register$username({
      email: values.email,
      ...
      <div style={{ width: 300 }}>
        <Form onFinish={onFinish} form={form}>
          ...
          <div style={{ display: 'flex' }}>
            ...
            <CommonSendButton
              form={form}
              sendEmailService={services.email$sendCode$register}
            />
          </div>
