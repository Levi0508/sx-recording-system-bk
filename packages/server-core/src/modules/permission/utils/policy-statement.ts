...
/*
 PolicyStatement 类的设计用于处理权限策略中的每个声明，
 提供了判断给定的操作（action）是否与策略匹配的能力。
 它通过 matchAction 方法结合 keyMatch 方法实现复杂的规则匹配。
*/
export class PolicyStatement implements IPolicyStatement {
  ...
  /**
   * 策略规则匹配，判断传入的 action 是否与当前策略声明中的任意 action 匹配。如果匹配成功，则返回true
   * @param action
   * @returns
   */
  matchAction(action: string): boolean {
    const result = this.action.some((_) => this.keyMatch(action, _));
    winstonService.info('PolicyStatement->matchAction', {
      action,
      result,
      effect: this.effect,
    });
  ...
  keyMatch(action: string, patter: string): boolean {
    winstonService.info('PolicyStatement->keyMatch', { action, patter });
