...
export class ReceptionService {
  ...
  async createSession(staffId?: number, agreementType: string = 'recording_privacy') {
    ...
    return { sessionId, token, tokenExpiresAt, agreementType };
  }
  ...
  async getStatus(sessionId: string): Promise<{ status: string; confirmedAt?: number } | null> {
    const session = await this.repo.findOne({
      where: { sessionId },
      select: ['status', 'confirmedAt'],
    });
    if (!session) return null;
    return { status: session.status!, confirmedAt: session.confirmedAt ?? undefined };
  }
}
