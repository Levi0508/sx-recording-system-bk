...
export class UserService {
  ...
  /**
   *   获取消费总金额最多的前10名用户及其消费总金额
   * @returns
   */
  async getTopThreeUserDetails() {
    // 获取消费总金额最多的前10名用户及其消费总金额
    const topUsers = await this.exchangeCardRepository
      .createQueryBuilder('exchange_card')
      .select('exchange_card.userId', 'userId')
      .addSelect('SUM(exchange_card.cardType)', 'payTotal')
      .groupBy('exchange_card.userId')
      .orderBy('payTotal', 'DESC')
      .limit(11)
      .getRawMany();
    this.logger.info('topUsers', topUsers);
    // 提取用户 ID
    ...
    const userDetails = await this.userRepository
      ...
      .where('user.id IN (:...ids)', { ids: topUserIds })
      .getMany();

    const detailedUsers = userDetails.map((user) => {
      const userPayTotal =
        topUsers.find((u) => u.userId === user.id)?.payTotal || 0;
      return {
        ...user,
        payTotal: userPayTotal * 10,
      };
    });

    // 按 payTotal 从大到小排序
    return detailedUsers.sort((a, b) => b.payTotal - a.payTotal);
  }
  /**
   * 查询单个用户的消费总金额
   * @param user 单个用户实体
   * @returns 消费总金额
   */
  async getUserPayDetails(user: UserEntity) {
    // 查询单个用户的消费总金额
    const userPayTotal = await this.exchangeCardRepository
      .createQueryBuilder('exchange_card')
      .select('SUM(exchange_card.cardType)', 'payTotal')
      .where('exchange_card.userId = :userId', { userId: user.id }) // 根据用户ID筛选
      .getRawOne();
    this.logger.info('锋酱的', userPayTotal);

    return userPayTotal ? userPayTotal.payTotal : 0; // 如果没有消费记录，返回0
  }
  ...
  async updateUserPointsBasedOnPayTotal() {
    // 获取所有有消费记录的用户及其消费总额
    const usersPayTotals = await this.exchangeCardRepository
      .createQueryBuilder('exchange_card')
      .select('exchange_card.userId', 'userId')
      .addSelect('SUM(exchange_card.cardType)', 'payTotal') // 计算每个用户的消费总额
      .groupBy('exchange_card.userId')
      .getRawMany();

    // 遍历每个用户，更新其积分
    for (const userPay of usersPayTotals) {
      const { userId, payTotal } = userPay;
  ...
  // async getTopThreeUserDetails() {
  //   // 获取消费总金额最多的前10名用户及其消费总金额
  //   const topUsers = await this.vipRepository
  //     .createQueryBuilder('vip')
  //     .select('vip.userId', 'userId')
  //     .addSelect('SUM(vip.price)', 'payTotal')
  //     .groupBy('vip.userId')
  //     .orderBy('payTotal', 'DESC')
  //     .limit(11)
  //     .getRawMany();
   ...
   * @returns
   */
  async wearHandler(user: UserEntity, avatarFrame?: AVATAR_TYPE_ENUM) {
    this.logger.info('锋酱的avatarFrame', { avatarFrame });

    await this.userRepository.update(
      { id: user.id },
      {
        avatarFrame: (avatarFrame ? avatarFrame : null) as any,
      },
    );
    return true;
  }

  async getAvatarFrame(userId: number) {
    const avatarFrames = await this.vipRepository.find({
      where: {
        userId,
        goodsType: 'avatar_frame',
      },
      // order: { createdAt: 'DESC' }, // 可以按创建时间排序
    });

    return avatarFrames.map((avatarFrame) => ({
      userId: avatarFrame.userId,
      avatarFrame: avatarFrame.name,
      price: avatarFrame.price,
      // expiryDate: avatarFrame.expiryDate,
      // isValid: avatarFrame.isValid(), // 添加有效性检查
    }));
  }
   ...
   * @param email
   */
  async banAccount(email: string, days: number, user: UserEntity) {
    // const user = await this.userRepository.findOneBy({ email });
    ...
    const now = dayjs();
    let newBanAccountDate: dayjs.Dayjs;

    if (user.banAccountDate && user.isBan()) {
      newBanAccountDate = dayjs(user.banAccountDate).add(days, 'day');
    } else {
      newBanAccountDate = now.add(days, 'day');
    }

    user.banAccountDate = newBanAccountDate.toDate();

    await this.userRepository.update(user.id, {
      banAccountDate: user.banAccountDate,
    });
    //顶号
   ...
   * @param email
   */
  async deblockingAccount(email: string) {
    const user = await this.userRepository.findOneBy({ email });
   ...
   * @param email
   */
  async convertVipToBalance(email: string) {
    const user = await this.userRepository.findOneBy({ email });
    ...
    // 检查是否已经折算过
    const existingConvert = await this.convertVipRepository.findOneBy({
      userId: user.id,
    });

    if (existingConvert) {
      throw new ServiceException('该用户已经折算过，不能重复折算');
    }

    // 如果是永久会员，不处理
    if (user.vipType === VIP_TYPE_ENUM.PERMANENT) {
      throw new ServiceException('永久会员暂不处理');
    }

    // 标准时间：2026/1/15 00:00:00（开始时间）
    const standardDate = dayjs('2026-01-15').startOf('day');

    // 如果没有VIP过期时间，不处理
    if (!user.vipDeadLine) {
      throw new ServiceException('用户没有VIP过期时间');
    }

    const vipDeadLine = dayjs(user.vipDeadLine);
    const vipDeadLineStartOfDay = vipDeadLine.startOf('day');

    // 如果VIP在2026/1/15之前已经过期，不补偿
    if (vipDeadLineStartOfDay.isBefore(standardDate)) {
      throw new ServiceException('2026/1/15之前已过期，不进行补偿');
    }

    // 计算有效天数：从标准时间（2026-01-15）到VIP过期时间的天数
    const days = vipDeadLineStartOfDay.diff(standardDate, 'day');

    if (days <= 0) {
      throw new ServiceException('没有可折算的有效天数');
    }

    // 按1天有效期返回1枚平台币（注意入库时的换算：money存储是实际值的10倍）
    const platformCoins = days; // 平台币数量（枚）
    const moneyToAdd = platformCoins * 10; // 转换为数据库存储格式

    // 使用Redis锁保证并发安全
    const redisClient = this.redisService.getClient();
    const lockKey = `user:${user.id}:convert:lock`;
    const acquiredLock = await redisClient.setnx(lockKey, 'locked');

    try {
      if (acquiredLock === 1) {
        await redisClient.expire(lockKey, 5); // 上锁5秒

        // 再次检查是否已经折算过（防止并发）
        const existingConvertCheck = await this.convertVipRepository.findOneBy({
          userId: user.id,
        });

        if (existingConvertCheck) {
          await redisClient.del(lockKey);
          throw new ServiceException('该用户已经折算过，不能重复折算');
        }
        ...
        // 插入折算记录
        const convertRecord = new ConvertVipEntity();
        convertRecord.userId = freshUser.id;
        convertRecord.email = email;
        convertRecord.vipDeadLine = user.vipDeadLine;
        convertRecord.days = days;
        convertRecord.platformCoins = platformCoins;
        convertRecord.moneyAdded = moneyToAdd;
        await this.convertVipRepository.save(convertRecord);
  ...
  async recordAnchorGoodsClick(
    ...
    const clickRecord = this.anchorGoodsClickRepository.create({
      userId,
      goodsId,
      url: url || undefined,
    });
    await this.anchorGoodsClickRepository.save(clickRecord);
  }
}
